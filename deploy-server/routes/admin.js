...
router.post("/admin/lessons", adminAuth, async (req, res) => {
  const { course_id, title, youtube_url, order_index } = req.body;
...
router.get("/admin/completions-detail", adminAuth, async (_req, res) => {
  try {
    ...
    const certSet = new Set(certRows.rows.map((r) => `${r.user_id}_${r.course_id}`));

    const studentProgress = enrolledStudents.rows.map((e) => {
    ...
    const courseSummaries = COURSE_IDS.map((cid) => {
      const enrolled = enrolledStudents.rows.filter((r) => r.course_id === cid);
      const total = lessonCountMap[cid] || 0;
      const completedAll = enrolled.filter((r) => (progressMap[`${r.user_id}_${cid}`] || 0) >= total && total > 0).length;
      const passedTest = enrolled.filter((r) => testMap[`${r.user_id}_${cid}`]?.passed).length;
      const certs = enrolled.filter((r) => certSet.has(`${r.user_id}_${cid}`)).length;
      return {
        course_id: cid,
