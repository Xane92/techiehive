...
router.post("/payment/initialize", async (req, res) => {
  const { email, amount, courseId, callbackUrl } = req.body;
...
router.get("/payment/verify/:reference", async (req, res) => {
  ...
  try {
    ...
    const courseId = parseInt(String(txData.metadata?.courseId || "0"), 10);
    const amountPaid = Math.round(txData.amount / 100);
